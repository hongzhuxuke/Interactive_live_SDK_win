...
#include "../vhall_audio_capture/DShowAudioCapture.h"
#include "../signalling/tool/DeviceObtain.h"
#include "../vhall_audio_capture/AudioCaptureMix.h"
...
namespace webrtc {
  ...
  class VHAudioDeviceWindowsCore : public AudioDeviceGeneric, public IDataReceiver {
  public:
    VHAudioDeviceWindowsCore();
    ~VHAudioDeviceWindowsCore();
    ...
    virtual void PushAudioSegment(unsigned char *buffer, unsigned int numFrames, unsigned long long timestamp) {};
    ...
    /* DShow capture */
    virtual int32_t SetRecordingDevice(std::string device);
    void SetAudioLayer(webrtc::AudioDeviceModule::AudioLayer audio_layer);
  ...
  public:
    ...
    void SetLoopBackVolume(uint32_t volume);
    int32_t RegisterAudioCaptureObserver(webrtc::AudioTransport* audio_observer);
    
    std::string mDshowDevice = "";
    webrtc::AudioDeviceModule::AudioLayer mAudioLayer;
    std::shared_ptr<CDeviceObtain> m_pDeviceObtain;
    std::shared_ptr<vhall::DShowAudioCapture> mDShowCapture;
    /* audio mix */
    std::shared_ptr<vhall::AudioCaptureMix> mMix;
    webrtc::PushResampler<int16_t> mResampler;
    std::shared_ptr<vhall::AudioLoopBackRecording> mLoopBack;
    std::atomic<float> mMicroPhoneVolume;
    WORD _recBitsPerSample = 0;
    vhall::AudioDeviceNotify* mDeviceNotify = nullptr;
    webrtc::AudioTransport* audio_observer = nullptr;
  ...
  private:  // thread functions
    ...
    void QuadToStereo(const int16_t* src_audio, size_t samples_per_channel, int16_t* dst_audio);
    ...
    std::string _TraceCOMError(HRESULT hr) const;
