...
void GetSampleProperties(IMediaSample* sample, AM_SAMPLE2_PROPERTIES* props) {
  ...
  if (SUCCEEDED(webrtc::videocapturemodule::GetComInterface(sample, &sample2))) {
...
class MediaTypesEnum : public IEnumMediaTypes {
      ...
      : capability_(capability),
        format_preference_order_(
            {// Default preferences, sorted by cost-to-convert-to-i420.
             webrtc::VideoType::kI420, webrtc::VideoType::kYUY2, webrtc::VideoType::kRGB24,
             webrtc::VideoType::kUYVY, webrtc::VideoType::kMJPEG}) {
 ...
 private:
  ...
  static void SetMediaInfoFromVideoType(webrtc::VideoType video_type,
  ...
  int pos_ = 0;
  const webrtc::VideoCaptureCapability capability_;
  std::list<webrtc::VideoType> format_preference_order_;
};
...
CaptureInputPin::~CaptureInputPin() {
  ...
  webrtc::videocapturemodule::ResetMediaType(&media_type_);
}
...
HRESULT CaptureInputPin::SetRequestedCapability(
  ...
  resulting_capability_ = webrtc::VideoCaptureCapability();
  return S_OK;
}
...
HRESULT CaptureInputPin::AttemptConnection(IPin* receive_pin,
  ...
  webrtc::videocapturemodule::ResetMediaType(&media_type_);
  webrtc::videocapturemodule::CopyMediaType(&media_type_, media_type);
    ...
    if (types) {
      while (true) {
        ...
        if (IsMediaTypePartialMatch(*this_type, *media_type)) {
          ...
          webrtc::videocapturemodule::FreeMediaType(this_type);
        }
      }
      types->Release();
    }
...
STDMETHODIMP CaptureInputPin::ConnectionMediaType(AM_MEDIA_TYPE* media_type) {
  ...
  webrtc::videocapturemodule::CopyMediaType(media_type, &media_type_);
...
STDMETHODIMP CaptureInputPin::QueryAccept(const AM_MEDIA_TYPE* media_type) {
  RTC_DCHECK_RUN_ON(&main_checker_);
  RTC_DCHECK(Filter()->IsStopped());
  webrtc::VideoCaptureCapability capability(resulting_capability_);
...
STDMETHODIMP CaptureInputPin::EnumMediaTypes(IEnumMediaTypes** types) {
  RTC_DCHECK_RUN_ON(&main_checker_);
  *types = new webrtc::videocapturemodule::ComRefCount<MediaTypesEnum>(requested_capability_);
  (*types)->AddRef();
  return S_OK;
}
...
CaptureSinkFilter::CaptureSinkFilter(webrtc::videocapturemodule::VHVideoCaptureImpl* capture_observer)
    : input_pin_(new webrtc::videocapturemodule::ComRefCount<CaptureInputPin>(this)),
...
STDMETHODIMP CaptureSinkFilter::EnumPins(IEnumPins** pins) {
  RTC_DCHECK_RUN_ON(&main_checker_);
  *pins = new webrtc::videocapturemodule::ComRefCount<class EnumPins>(input_pin_.get());
  (*pins)->AddRef();
  return S_OK;
}
...
STDMETHODIMP CaptureSinkFilter::JoinFilterGraph(IFilterGraph* graph,
  ...
  if (info_.pGraph) {
    ...
    webrtc::videocapturemodule::GetComInterface(info_.pGraph, &sink);
    sink_ = sink.get();
  }
