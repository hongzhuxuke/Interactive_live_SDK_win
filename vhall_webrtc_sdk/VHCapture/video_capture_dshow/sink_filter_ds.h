...
class CaptureInputPin : public IMemInputPin, public IPin {
 ...
 private:
  ...
  webrtc::VideoCaptureCapability requested_capability_ RTC_GUARDED_BY(main_checker_);
  // Accessed on the main thread when Filter()->IsStopped() (capture thread not
  // running), otherwise accessed on the capture thread.
  webrtc::VideoCaptureCapability resulting_capability_;
...
class CaptureSinkFilter : public IBaseFilter {
 public:
  CaptureSinkFilter(webrtc::videocapturemodule::VHVideoCaptureImpl* capture_observer);
 ...
 private:
  rtc::ThreadChecker main_checker_;
  const rtc::scoped_refptr<webrtc::videocapturemodule::ComRefCount<CaptureInputPin>> input_pin_;
  webrtc::videocapturemodule::VHVideoCaptureImpl* const capture_observer_;
